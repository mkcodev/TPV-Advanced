...
- [ ] **A6. 🖥️ Crea una cuenta en Supabase** y un **proyecto nuevo**. Guarda: la *Project URL*, la *anon key*, la *service_role key* y la *connection string* de la base de datos (`DATABASE_URL`).
...
- [ ] **B2. 🖥️ Crea el archivo `.env`** copiando `.env.example`, y pega tus credenciales de Supabase (paso A6):
  ```bash
...
- [ ] **D2. 🧠 Tarea 0.2** — Materializar el esquema completo en Drizzle (plan → código → migración, sin aplicar).
- [ ] **D3. 🖥️ Aplica la migración a Supabase:** con `DATABASE_URL` en el `.env`, ejecuta `pnpm db:migrate`. Verifica en el panel de Supabase que aparecen las tablas.
- [ ] **D4. 🧠 Tarea 0.3** — Conexión a Supabase + políticas RLS multi-negocio.
- [ ] **D5. 🧠 Tarea 0.4** — Autenticación en dos niveles (admin por email + empleado por PIN).
- [ ] **D6. 🧠 Tarea 0.5** — **Design system** (shadcn con nuestros tokens). *Antes de cualquier pantalla.*
- [ ] **D7. ⚡ Tarea 0.6** — Armazón de UI + i18n (solo estructura, con `@tpv/ui`).
- [ ] **D8. 🧠 Tarea 0.7** — Storybook en `packages/ui` (Button + story de tokens).
- [ ] **D9. ✅ Checkpoint Fase 0:** el proyecto arranca (`pnpm dev`), la BD existe con RLS, puedes iniciar sesión, ves el armazón del TPV y Storybook funciona. Commit.
...
- [ ] **E1. ⚡ Tarea 1.1** — Backend del catálogo (tRPC + Drizzle).
- [ ] **E2. ⚡ Tarea 1.2** — Panel admin del catálogo + subida de fotos.
- [ ] **E3. ⚡ Tarea 1.3** — Pantalla del TPV: tocar producto → total en vivo.
- [ ] **E4. ⚡ Tarea 1.4** — Variantes, modificadores y menús.
- [ ] **E5. ⚡ Tarea 1.5** — Plano visual de sala y mesas.
- [ ] **E6. 🧠 Tarea 1.6** — Persistir comandas y aparcarlas en mesas.
- [ ] **E7. 🧠 Tarea 1.7** — Dividir cuenta, juntar mesas, transferir.
- [ ] **E8. 🧠 Tarea 1.8** — Cobro (efectivo/tarjeta/mixto) y cierre de comanda.
- [ ] **E9. ⚡ Tarea 1.9** — Ticket y comanda de cocina imprimibles (local).
- [ ] **E10. ⚡ Tarea 1.10** — Empleados, login por PIN y ventas por empleado.
- [ ] **E11. ⚡ Tarea 1.11** — Sesión de caja y cierre Z (arqueo).
...
- [ ] **F1. FASE 2** — Veri*factu (🧠, lo más crítico) → tiempo real → app móvil Expo → cocina KDS/impresora → empaquetado Electron → offline. *Cierra el checkpoint antes de la Fase 3.*
- [ ] **F2. FASE 3** — SaaS: onboarding multi-negocio → informes → inventario → suscripciones Stripe → multi-local.
- [ ] **F3. FASE 4** — Diferenciadores: carta QR → pago en mesa → fidelización → reservas → delivery.
...
- 🧠 Opus para arquitectura, esquema, lo legal (Veri*factu) y todo lo que reparte dinero.
- Dinero en céntimos, `business_id` en toda consulta, tablas legales solo-añadir, textos por i18n.
- UI: solo tokens del `DESIGN-SYSTEM.md`; nada inventado.
...
6. `docs/PLANIFICACION-TPV.md` — la visión y el "porqué".
7. `docs/CLAUDE-CODE-SETUP.md` — detalle de la puesta en marcha de Claude Code.
